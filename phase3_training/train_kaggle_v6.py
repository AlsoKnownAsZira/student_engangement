"""
=============================================================================
 TRAINING SCRIPT v6 - Kaggle (YOLO Classify, 3-class crops)
=============================================================================
 Perubahan arsitektur vs v1-v5 (detection):
   - Task : YOLO CLASSIFY (bukan detect) — satu model, satu crop = satu kelas
   - Data : crops_v6/ per-person crops, balanced 10k/kelas di train
   - Model: yolo11s-cls  (model lebih kecil OK — classify jauh lebih sederhana
             dari detect; v1-v4 buktikan model size bukan bottleneck)

 Keputusan dari rangkuman v1-v4:
   - Plateau 54-59% di v1-v4 bukan soal model/resolusi, tapi data.
     Sekarang data diserang langsung: crop per orang + perfectly balanced.
   - patience=15 (v1) terlalu agresif → pakai 50
   - label_smoothing=0.1 → pertahankan (tidak ada kerugian dari v4)
   - Resolusi 224px cukup untuk crop orang (raw crop ~100-300px dari 2560px)
   - Augmentasi online: mosaic dan copy_paste tidak relevan untuk classify,
     andalkan flip + color jitter + random erasing bawaan Ultralytics classify

 Setup Kaggle:
   1. Zip folder crops_v6/ secara lokal
...
ZIP_PATH  = WORKING / "crops_v6.zip"
DATA_DIR  = WORKING / "crops_v6"
...
def check_dataset():
    ...
    print("\n" + "=" * 55)
    print("DISTRIBUSI DATASET V6 (crop-based classify)")
    print("=" * 55)
    for split in ["train", "valid", "test"]:
        ...
        for cls in classes:
            ...
            bar = "█" * int(pct / 3)
            print(f"    {cls:8s}: {counts[cls]:6d} ({pct:5.1f}%)  {bar}")
    print("=" * 55)
...
def train():
    ...
    print("\n" + "=" * 55)
    print("TRAINING v6 — YOLO Classify (3-class crops)")
    print("=" * 55)
    print(f"  Task    : classify")
    print(f"  Model   : yolo11s-cls")
    print(f"  Classes : High / Low / Medium")
    print(f"  imgsz   : 224px  (crop orang, raw ~100-300px)")
    print(f"  Epochs  : 100  (patience 50)")
    print(f"  Batch   : 64   (classify lebih ringan dari detect)")
    print("=" * 55 + "\n")
    ...
    results = model.train(
        ...
        # === CORE ===
        epochs=100,
        imgsz=224, # Size dari cropped kecilan
        batch=64,
        device=0,
        workers=4,
        cache="disk",
        ...
        # === EARLY STOPPING ===
        # v1 pakai patience=15 → terlalu agresif → naik ke 50
        patience=50,
        ...
        # === AUGMENTASI ONLINE ===
        # Mosaic/copy_paste tidak relevan untuk classify task
        # Ultralytics classify pakai RandomResizedCrop + ColorJitter + RandAugment
        fliplr=0.5,
        flipud=0.0,
        degrees=10.0,
        translate=0.1,
        ...
        # === REGULARISASI ===
        # label_smoothing membantu sedikit di v4, pertahankan
        label_smoothing=0.1,
        weight_decay=0.0005,
        dropout=0.2,
...
def per_class_report(best_pt: str):
    """
    Jalankan inferensi manual pada test set → sklearn classification_report.
    Ini memberikan per-class Precision, Recall, F1 yang tidak ada di val().
    """
...
def evaluate(best_pt: str):
    ...
    print("\n" + "=" * 55)
    print("HASIL EVALUASI V6 — Top-1 Accuracy")
    print("=" * 55)
    ...
    # Per-class breakdown pakai sklearn
...
if __name__ == "__main__":
    ...
    best = str(RUN_DIR / "engagement_v6_classify" / "weights" / "best.pt")
    evaluate(best)
