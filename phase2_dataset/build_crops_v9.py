"""
Build crops_v9: crop dari dataset_smp_balanced → 2-class balanced.

Perbedaan dari V8:
  - Source: dataset_smp_balanced (Kelas9_2mar_1012 dipindah train→valid)
  - Valid set menjadi ~50:50 Engaged:NotEngaged (1805:1813)
    sehingga best.pt dipilih berdasarkan metrik yang tidak bias
  - Train Low berkurang: 6130→5336, augment offline ke ~8398 (~57% synthetic)
  - Hyperparameter training: AdamW, lr0=0.0005, imgsz=320, device="0,1"

Alur:
  1. Crop setiap bbox dari dataset_smp_balanced/{split}/images+labels
...
Output: phase2_dataset/crops_v9/{train,valid,test}/{Engaged,NotEngaged}/
"""
...
SRC    = ROOT / "dataset_smp_balanced"
DST    = ROOT / "phase2_dataset" / "crops_v9"
...
def crop_split_to_dirs(split: str, out_dirs: dict) -> dict:
    ...
    cropped = {c: [] for c in CLASS_MAP.values()}
    skipped = 0
...
def augment_to_target(src_files: list, dst_dir: Path, target: int):
...
def build_split(split: str):
    ...
    print(f"\n[{split.upper()}] Cropping dari dataset_smp_balanced ...")
...
def main():
    if not SRC.exists():
        print(f"[ERROR] Source tidak ditemukan: {SRC}")
        print(f"Jalankan dulu: python phase2_dataset/resplit_balanced.py")
        return
    ...
    print(f"\nSelesai. crops_v9 siap di-zip dan upload ke Kaggle.")
    print(f"Lokasi: {DST}")
